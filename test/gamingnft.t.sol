...
contract GamingNFtTest is Test {
    ...
    function testInsufficientETHForSingleMint() public {
        ...
        assertEq(
            gamingNFT.balanceOf(recipient, tokenId),
            0,
            "Recipient's balance should not increase due to insufficient ETH"
        );
    }
    ...
    function testSuccessfulBatchMint() public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            assertEq(
                gamingNFT.balanceOf(recipient, tokenIds[i]), amounts[i], "Batch balance did not increase as expected"
            );
        }
    }
    ...
    function testSupplyManagement() public {
        ...
        assertEq(
            gamingNFT.totalSupply(tokenId),
            amount + additionalAmount,
            "Total supply should be updated to include additional minted tokens"
        );
        ...
        assertEq(
            gamingNFT.totalSupply(tokenIds[0]),
            amount + additionalAmount + amounts[0],
            "Total supply for tokenId 1 should be updated after batch mint"
        );
        assertEq(
            gamingNFT.totalSupply(tokenIds[1]),
            amounts[1],
            "Total supply for tokenId 2 should be equal to the amount minted in batch"
        );
    }
}
